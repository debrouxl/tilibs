...
int ti9x_dup_Regular(Ti9xRegular *dst, Ti9xRegular *src)
{
  ...
  for(i=0; i<src->num_entries; i++)
    TRY(ti9x_dup_VarEntry(&(dst->entries[i]),
		      &(src->entries[i])));
...
*/
int ti9x_free_regular_content(Ti9xRegular *content)
{
  int i;
...
int ti9x_free_backup_content(Ti9xBackup *content)
{
  free(content->data_part);
  return 0;
}

int ti9x_free_flash_content(Ti9xFlash *content)
{
  Ti9xFlash *ptr;
 ...
 */
int ti9x_read_regular_file(const char *filename, 
			   Ti9xRegular *content)
{
  FILE *f;
  long cur_pos = 0;
...
/*
  ...
  - int [out]: an error code
*/
int ti9x_read_backup_file (const char *filename, 
					   Ti9xBackup  *content)
{
  FILE *f;
  uint32_t file_size;
...
int ti9x_read_flash_file(const char *filename,
                                         Ti9xFlash *head)
{
  int tib = 0;
  FILE *f;
...
/*
  ...
  - int [out]: an error code
*/
int ti9x_write_regular_file(const char *fname, 
			    Ti9xRegular *content,
			    char **real_fname)
{
  FILE *f;
...
/*
  ...
  - int [out]: an error code
*/
int ti9x_write_backup_file (const char *filename, 
					    Ti9xBackup  *content)
{
  FILE *f;
...
/* 
   ...
   - int [out]: an error code
*/
int ti9x_display_regular_content(Ti9xRegular *content)
{
  int i;
  char trans[9];
  ...
  tprintf("Comment:           <%s>\n", content->comment);
  tprintf("Default folder:    <%s>\n", content->default_folder); 
  tprintf("Number of entries: %i\n", content->num_entries);
...
/*
  ...
  - int [out]: an error code
*/
int ti9x_display_backup_content(Ti9xBackup *content)
{
  tprintf("signature:      <%s>\n",
	  tifiles_calctype2signature(content->calc_type));
  tprintf("comment:        <%s>\n", content->comment);
  tprintf("ROM version:    <%s>\n", content->rom_version);
  tprintf("type:           %02X (%s)\n",
         content->type, tifiles_vartype2string(content->type));
  tprintf("data length:    %08X (%i)\n",
...
/*
  ...
  - int [out]: an error code
*/
int ti9x_display_flash_content(Ti9xFlash *content)
{
  Ti9xFlash *ptr;
  ...
  for(ptr = content; ptr != NULL; ptr = ptr->next)
    {
      ...
      tprintf("Name:           <%s>\n", ptr->name);
      tprintf("Device type:    %s\n", ptr->device_type == DEVICE_TYPE_89 ?
	     "ti89" : "ti92+");
      tprintf("Data type:      ");

...
extern "C" {
  ...
  TIEXPORT void TICALL tifiles_set_calc(int type);
  TIEXPORT int  TICALL tifiles_get_calc(void);
  ...
  TIEXPORT const char TICALL *tifiles_attribute_to_string(int var_atrb);
  TIEXPORT const char TICALL *tifiles_vartype_to_file_extension(int var_type);
  ...
  TIEXPORT const char* TICALL tifiles_calctype2signature(int calc_type);
  TIEXPORT int         TICALL tifiles_signature2calctype(const char *signat);
  ...
  int   TICALL tifiles_build_fullname(char* full_name, const char *fldname, 
				      const char* varname);
  
  // filesXX.c: layer built on tifiles8x/9x

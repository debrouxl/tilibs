...
static int		recv_screen	(CalcHandle* handle, CalcScreenCoord* sc, uint8_t** bitmap)
{
	...
	ret = dusb_cmd_s_param_request(handle, sizeof(pids) / sizeof(pids[0]), pids);
	if (!ret)
	{
		uint32_t size;
		uint8_t *data;
...
static int		get_dirlist	(CalcHandle* handle, GNode** vars, GNode** apps)
{
	...
	RETURN_ERR_MALLOC_IF_NULLPTR(folder);
	...
	RETURN_ERR_MALLOC_IF_NULLPTR(node);
	...
	if (!ret)
	{
		for (;;)
		{
			...
			ve->size = (  (((uint32_t)(attrs[0].data[0])) << 24)
			            | (((uint32_t)(attrs[0].data[1])) << 16)
			            | (((uint32_t)(attrs[0].data[2])) <<  8)
			            | (((uint32_t)(attrs[0].data[3]))      ));
			ve->type = (uint32_t)(attrs[1].data[3]);
			ve->attr = attrs[2].data[0] ? ATTRB_ARCHIVED : ATTRB_NONE;
			dusb_ca_del_array2(handle, attrs, naids, 1);
...
static int		send_backup	(CalcHandle* handle, BackupContent* content)
{
	...
	uint32_t length = content->data_length1 + content->data_length2 + content->data_length3;
	uint8_t * data = (uint8_t *)g_malloc(length);
	RETURN_ERR_MALLOC_IF_NULLPTR(data);
	memcpy(data, content->data_part1, content->data_length1);
	memcpy(data + content->data_length1, content->data_part2, content->data_length2);
	memcpy(data + content->data_length1 + content->data_length2, content->data_part3, content->data_length3);

	// enter manual link mode
	for (unsigned int i = 0; !ret && i < sizeof(keys) / sizeof(keys[0]) - 1; i++)
	{
		ret = send_key(handle, (uint32_t)(keys[i]));
	}
	if (!ret)
	{
		ret = send_key_noack(handle, (uint32_t)(keys[sizeof(keys) / sizeof(keys[0]) - 1]));
		if (!ret)
		{
			uint8_t attr0data[4] = { 0xF0, 0x07, 0x00, TI84p_BKUP };
			uint8_t attr1data[4] = { 0x00, 0x00, 0x00, content->version };
			uint8_t attr2data[8] = {
				MSB(content->data_length1), LSB(content->data_length1),
				MSB(content->data_length2), LSB(content->data_length2),
				MSB(content->data_length3), LSB(content->data_length3),
				MSB(content->mem_address),  LSB(content->mem_address)
			};

			const DUSBCalcAttr attrs[3] = {
				{ DUSB_AID_VAR_TYPE,      0, sizeof(attr0data), attr0data },
				{ DUSB_AID_VAR_VERSION,   0, sizeof(attr1data), attr1data },
				{ DUSB_AID_BACKUP_HEADER, 0, sizeof(attr2data), attr2data }
			};
			// send backup header
			ret = dusb_cmd_s_rts_ns(handle, "", "!", length, sizeof(attrs) / sizeof(attrs[0]), attrs);
			if (!ret)
			{
				ret = dusb_cmd_r_delay_ack(handle);
				if (!ret)
...
static int		send_var	(CalcHandle* handle, CalcMode mode, FileContent* content)
{
	...
	for (unsigned int i = 0; !ret && i < content->num_entries; i++)
	{
		...
		uint8_t attr0data[4] = { 0xF0, 0x07, 0x00, entry->type };
		uint8_t attr1data[1] = { entry->attr == ATTRB_ARCHIVED ? (uint8_t)1 : (uint8_t)0 };
		uint8_t attr2data[4] = { 0x00, 0x00, 0x00, entry->version };
		...
		ret = dusb_cmd_s_rts(handle, "", entry->name, size, sizeof(attrs) / sizeof(attrs[0]), attrs);
		if (!ret)
		{
			ret = dusb_cmd_r_data_ack(handle);
			if (!ret)
...
static int		recv_var	(CalcHandle* handle, CalcMode mode, FileContent* content, VarRequest* vr)
{
	...
	uint8_t attr0data[4] = { 0xF0, 0x07, 0x00, vr->type };
	...
	if (!ret)
	{
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				...
				if (!ret)
				{
					if (/*   !attrs2[0].ok || attrs2[0].size < 1
					    || !attrs2[1].ok || attrs2[1].size < 4
					    ||*/ !attrs2[2].ok || attrs2[2].size < 4
					   )
					{
						ret = ERR_INVALID_PACKET;
					}
					else
					{
						...
						ve->size = (  (((uint32_t)(attrs2[2].data[0])) << 24)
							    | (((uint32_t)(attrs2[2].data[1])) << 16)
							    | (((uint32_t)(attrs2[2].data[2])) <<  8)
							    | (((uint32_t)(attrs2[2].data[3]))      ));

						if (attrs2[0].ok && attrs2[0].size == 1)
						{
							ve->attr = (attrs2[0].data[0] ? ATTRB_ARCHIVED : 0);
						}
						if (attrs2[1].ok && attrs2[1].size == 4)
						{
							ve->version = attrs2[1].data[3];
						}
...
static int		send_flash	(CalcHandle* handle, FlashContent* content)
{
	...
	uint32_t size = ptr->num_pages * FLASH_PAGE_SIZE;
	uint8_t * data = (uint8_t *)tifiles_fp_alloc_data(size);	// must be rounded-up
	RETURN_ERR_MALLOC_IF_NULLPTR(data);
	...
	uint8_t attr0data[4] = { 0xF0, 0x07, 0x00, ptr->data_type };
	uint8_t attr1data[1] = { 0x00 };
	const DUSBCalcAttr attrs[2] = {
		{ DUSB_AID_VAR_TYPE, 0, sizeof(attr0data), attr0data },
		{ DUSB_AID_ARCHIVED, 0, sizeof(attr1data), attr1data },
	};

	ret = dusb_cmd_s_rts(handle, "", ptr->name, size, sizeof(attrs) / sizeof(attrs[0]), attrs);
	if (!ret)
	{
		ret = dusb_cmd_r_data_ack(handle);
		if (!ret)
...
static int		send_flash_834pce	(CalcHandle* handle, FlashContent* content)
{
	...
	uint8_t attr0data[4] = { 0xF0, 0x0F, 0x00, ptr->data_type };
	uint8_t attr1data[1] = { 0x01 };
	const DUSBCalcAttr attrs[2] = {
		{ DUSB_AID_VAR_TYPE, 0, sizeof(attr0data), attr0data },
		{ DUSB_AID_ARCHIVED, 0, sizeof(attr1data), attr1data },
	};
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				...
				if (!ret)
				{
					...
					int q = data_length / FLASH_PAGE_SIZE;
					int r = data_length % FLASH_PAGE_SIZE;
...
static int		recv_flash_834pce	(CalcHandle* handle, FlashContent* content, VarRequest* vr)
{
	...
	uint8_t attr0data[4] = { 0xF0, 0x0F, 0x00, vr->type };
	const DUSBCalcAttr attrs[1] = {
		{ DUSB_AID_VAR_TYPE2, 0, sizeof(attr0data), attr0data }
	};
...
static int		send_os    (CalcHandle* handle, FlashContent* content)
{
	...
	for (unsigned i = 0; i < ptr->num_pages; i++)
	{
		FlashPage *fp = ptr->pages[i];
	...
	if (!ret)
	{
		...
		for (unsigned i = 0; !ret && i < ptr->num_pages; i++)
		{
			FlashPage *fp = ptr->pages[i];

			fp->addr = 0x4000;

			if (i == 0) // needs relocation
			{
			...
			{
				for (unsigned int j = 0; !ret && j < fp->size; j += 256)
				{
...
static int		send_os_834pce    (CalcHandle* handle, FlashContent* content)
{
	...
	{
		// send OS data
		int q = ptr->data_length / (pkt_size - 4);
		int r = ptr->data_length % (pkt_size - 4);
...
static int		recv_idlist	(CalcHandle* handle, uint8_t* id)
{
	...
	uint8_t attr0data[4] = { 0xF0, 0x07, 0x00, TI83p_IDLIST };
	const DUSBCalcAttr attrs[1] = {
		{ DUSB_AID_VAR_TYPE2, 0, sizeof(attr0data), attr0data }
	};

	ret = dusb_cmd_s_var_request(handle, "", "IDList", naids, aids, sizeof(attrs) / sizeof(attrs[0]), attrs);
	if (!ret)
	{
		char fldname[40], varname[40];
		DUSBCalcAttr * attrs2 = dusb_ca_new_array2(handle, naids);
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				...
				if (!ret)
				{
					...
					for (uint32_t i = 4; i < varsize && i < 16; i++)
					{
...
static int		dump_rom_1	(CalcHandle* handle)
{
	...
	if (!ret)
	{
		...
		{
			ret = rd_send(handle, "romdump.8Xp", romDumpSize82a, romDump82a);
		}
...
{
	static const uint16_t pids[] = { DUSB_PID_CLASSIC_CLK_SUPPORT, DUSB_PID_NEW_CLK_SUPPORT };
	const unsigned int npids = sizeof(pids) / sizeof(pids[0]);
	int ret;
...
static int		del_var		(CalcHandle* handle, VarRequest* vr)
{
	...
	uint8_t attr0data[4] = { 0xF0, 0x0B, 0x00, vr->type };
	const DUSBCalcAttr attrs[1] = {
		{ DUSB_AID_VAR_TYPE2, 0, sizeof(attr0data), attr0data }
	};

	ret = dusb_cmd_s_var_delete(handle, "", vr->name, sizeof(attrs) / sizeof(attrs[0]), attrs);
	if (!ret)
	{
		ret = dusb_cmd_r_data_ack(handle);
	}
...
static int		rename_var	(CalcHandle* handle, VarRequest* oldname, VarRequest* newname)
{
	...
	utf81 = ticonv_varname_to_utf8(handle->model, oldname->name, oldname->type);
	utf82 = ticonv_varname_to_utf8(handle->model, newname->name, newname->type);
	...
	uint8_t attr0data[4] = { 0xF0, 0x07, 0x00, oldname->type };
	const DUSBCalcAttr attrs[1] = {
		{ DUSB_AID_VAR_TYPE2, 0, sizeof(attr0data), attr0data }
	};

	ret = dusb_cmd_s_var_modify(handle, "", oldname->name, sizeof(attrs) / sizeof(attrs[0]), attrs, "", newname->name, 0, nullptr);
	if (!ret)
	{
		ret = dusb_cmd_r_data_ack(handle);
	}
...
static int		change_attr	(CalcHandle* handle, VarRequest* vr, FileAttr attr)
{
	...
	utf8 = ticonv_varname_to_utf8(handle->model, vr->name, -1);
	ticalcs_slprintf(handle->updat->text, sizeof(handle->updat->text), _("Changing attributes of %s..."), utf8);
	ticonv_utf8_free(utf8);
	ticalcs_update_label(handle);


	uint8_t srcattr0data[4] = { 0xF0, 0x07, 0x00, vr->type };
	...
	uint8_t dstattr0data[1] = { (uint8_t)(attr == ATTRB_ARCHIVED ? 0xff : 0x00) };
	const DUSBCalcAttr dstattrs[1] = {
		{ DUSB_AID_ARCHIVED, 0, sizeof(dstattr0data), dstattr0data }
	};

	ret = dusb_cmd_s_var_modify(handle, "", vr->name, sizeof(srcattrs) / sizeof(srcattrs[0]), srcattrs, "", vr->name, sizeof(dstattrs) / sizeof(dstattrs[0]), dstattrs);
	if (!ret)
	{
		ret = dusb_cmd_r_data_ack(handle);
	}

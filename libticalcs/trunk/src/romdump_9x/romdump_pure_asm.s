...
|  Copyright (c) 2003-2018, Lionel Debroux for the optimized hardware detection code, the optimized ROM dumper code and the subsequent, compiler-assisted conversion to pure ASM.
|
...
.ifndef TIGCC_ENV
	.global __header, __header_end
	.global __footer, __footer_end, __end_of_program_data

	.section .ti68kheader1, "da"
	.even
__header:
| Magic number
	.ascii "**TI92**"
...
| Description, unused
	...
	.ascii "Compiled with generic toolchain\0\0\0\0\0\0\0\0\0"
| Filler 2
	.byte 0x01
	.byte 0x00
| Header size
...
| File name
	.ascii "romdump\0"
| Data type: TI-Basic program
	.byte 0x12
	.byte 0x00
	.byte 0x00
	.byte 0x00

| Generated in the linker script.
| Complete file size, including checksum, stored as 32-bit little-endian value: everything between __header and __footer_end.
|	.byte 0x00
|	.byte 0x00
|	.byte 0x00
|	.byte 0x00

	.section .ti68kheader2, "da"
| Filler 3
	.word 0xA55A
	.long 0x00000000

| Generated in the linker script.
| Program data size: everything between __header_end and __end_of_program_data.
	|.byte 0x00
	|.byte 0x00

	.section .ti68kheader3, "da"
__header_end:
...
| Although some hunks of the boot code support such a thing, sadly, no known real-world 89T has more than 4 MB of Flash memory...
	moveq    #0x40,%d6              |
	swap     %d6                    | ROM_size = 0x400000

.else

	cmpi.l   #0x800000,%d7          | ROM_base == 0x800000 ?
	beq.s    __rom_size_is_4M
| No, we don't have a 89T.
	cmp.l    #0x200000,%d7          | ROM_base == 0x200000 ?
	bne.s    __rom_size_is_2M
| We have either a V200 or a 89.
	move.l   ScrRect(%a5),%a0
	cmpi.b   #-56,2(%a0)            | 200
	bls.s    __rom_size_is_2M
__rom_size_is_4M:
| We have a 89T (because ROM_base == 0x800000) or a V200 (because ScrRect[2] > 200).
	moveq    #0x40,%d6
	bra.s    __end_rom_size_determination
__rom_size_is_2M:
| We have a 92+ (because ROM_base != 0x800000 && ROM_base != 0x200000) or a 89 (because ROM_base != 0x800000 && ScrRect[2] <= 200).
	moveq    #0x20,%d6
...
.else
| It's so refreshingly easy when one can use ROM_CALLs...
...
__end_of_program_data:
	.byte 0x00 | Checksum, will be computed again later... or maybe not.
	.byte 0x00
__footer_end:

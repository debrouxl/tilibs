...
TIEXPORT3 int TICALL nsp_cmd_r_login(CalcHandle * handle)
{
	...
	NSPVirtualPacket pkt { 0, 0, 0, 0, 0, 0, nullptr };
	ret = RECV_DATA(handle, &pkt);
	nsp_vtl_pkt_free_data(pkt.data);
...
TIEXPORT3 int TICALL nsp_cmd_r_screen_rle(CalcHandle * handle, uint8_t * cmd, uint32_t * size, uint8_t ** data)
{
	...
	NSPVirtualPacket pkt { 0, 0, 0, 0, 0, *size, nullptr };
	ret = RECV_DATA(handle, &pkt);
	if (!ret)
	{
		*cmd = pkt.cmd;
...
TIEXPORT3 int TICALL nsp_cmd_s_dir_attributes(CalcHandle * handle, const char * name)
{
	...
	uint32_t len = strlen(name) < 8 ? 8 : strlen(name);
	if (len > FULNAME_MAX)
	{
		return ERR_INVALID_PARAMETER;
	}
	uint8_t * data = (uint8_t *)nsp_vtl_pkt_alloc_data(1 + len + 1);
	if (nullptr == data)
	{
		return ERR_MALLOC;
	}
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_ATTRIBUTES, 1 + len + 1, data };
	pkt.data[0] = 0x01;
...
TIEXPORT3 int TICALL nsp_cmd_s_dir_enum_init(CalcHandle *handle, const char *name)
{
	...
	uint8_t * data = (uint8_t *)nsp_vtl_pkt_alloc_data(len + 1);
	if (nullptr == data)
	{
		return ERR_MALLOC;
	}
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_DIRLIST_INIT, len + 1, data };
...
TIEXPORT3 int TICALL nsp_cmd_s_put_file(CalcHandle *handle, const char *name, uint32_t size)
{
	...
	uint8_t * data = (uint8_t *)nsp_vtl_pkt_alloc_data(6 + len);
	if (nullptr == data)
	{
		return ERR_MALLOC;
	}
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_PUT_FILE, 6 + len, data };
	pkt.data[0] = 0x01;
...
TIEXPORT3 int TICALL nsp_cmd_s_get_file(CalcHandle *handle, const char *name)
{
	...
	uint8_t * data = (uint8_t *)nsp_vtl_pkt_alloc_data(2 + len);
	if (nullptr == data)
	{
		return ERR_MALLOC;
	}
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_GET_FILE, 2 + len, data };
	pkt.data[0] = 0x01;
	put_str(pkt.data + 1, name);
...
TIEXPORT3 int TICALL nsp_cmd_s_del_file(CalcHandle *handle, const char *name)
{
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_DEL_FILE, 2 + len, data };
	pkt.data[0] = 0x01;
	put_str(pkt.data + 1, name);
...
TIEXPORT3 int TICALL nsp_cmd_s_new_folder(CalcHandle *handle, const char *name)
{
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_NEW_FOLDER, 2 + len, data };
	pkt.data[0] = 0x03;
	put_str(pkt.data + 1, name);
...
TIEXPORT3 int TICALL nsp_cmd_s_del_folder(CalcHandle *handle, const char *name)
{
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_DEL_FOLDER, 2 + len, data };
	pkt.data[0] = 0x03;
	put_str(pkt.data + 1, name);
...
TIEXPORT3 int TICALL nsp_cmd_s_copy_file(CalcHandle *handle, const char *name, const char *name2)
{
	...
	uint32_t len = strlen(name) < 8 ? 8 : strlen(name);
	uint32_t len2 = strlen(name2) < 8 ? 8 : strlen(name2);
	if (len > FULNAME_MAX || len2 > FULNAME_MAX)
	{
		return ERR_INVALID_PARAMETER;
	}
	uint8_t * data = (uint8_t *)nsp_vtl_pkt_alloc_data(3 + len + len2);
	if (nullptr == data)
	{
		return ERR_MALLOC;
	}
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_COPY_FILE, 3 + len + len2, data };
	pkt.data[0] = 0x01;
	put_str(pkt.data + 1, name);
...
TIEXPORT3 int TICALL nsp_cmd_s_rename_file(CalcHandle *handle, const char *name, const char *name2)
{
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, NSP_PORT_FILE_MGMT, NSP_CMD_FM_RENAME_FILE, 3 + len + len2, data };
	pkt.data[0] = 0x01;
	put_str(pkt.data + 1, name);
	put_str(pkt.data + 2 + len, name2);
...
TIEXPORT3 int TICALL nsp_cmd_s_generic_data(CalcHandle *handle, uint32_t size, uint8_t *data, uint16_t sid, uint8_t cmd)
{
	...
	uint8_t * data2 = (uint8_t *)nsp_vtl_pkt_alloc_data(size);
	if (nullptr == data2)
	{
		return ERR_MALLOC;
	}
	...
	NSPVirtualPacket pkt { NSP_SRC_ADDR, handle->priv.nsp_src_port, NSP_DEV_ADDR, sid, cmd, size, data2 };
	if (data)
	{

...
int ti73_directorylist(TNode **tree, uint32_t *memory)
{
  ...
  TRYF(ti73_recv_XDP(&unused, (uint8_t *)memory)); fixup(*memory);
  TRYF(ti73_send_ACK());
  ...
  for( ; ; )
    {
      ...
      err = ti73_recv_VAR((uint16_t *)&ve->size, &ve->type, ve->name,
                          &ve->attr); fixup(ve->size);
      TRYF(ti73_send_ACK());
      if(err == ERR_EOT) 
	break;
      else if(err != 0) 
	...
	g_node_append(folder, node);
      ...
      sprintf(update->label_text, "Reading of: %s", ve->trans);
      update_label();
  ...
  for(i=0; i<content.num_entries; i++)
    {
      ...
      TRYF(ti73_send_RTS(entry->size, entry->type, varname, entry->attr));
      TRYF(ti73_recv_ACK(NULL));
...
int ti73_send_flash(const char *filename, int mask_mode)
{
  ...
  if(mask_mode & MODE_APPS)
    { // inform TI that a FLASH app is about to be sent
      uint16_t length;
      uint8_t buffer[16];
  ...
  for(i=0; i<content.num_pages; i++)
    {
      ...
      TRYF(ti73_send_VAR2(fp->length, content.data_type, fp->flag,
			  fp->offset, fp->page));
      TRYF(ti73_recv_ACK(NULL));
...
int ti73_recv_flash(const char *filename, int mask_mode, TiVarEntry *ve)
{
  ...
  content->pages = (Ti8xFlashPage *)calloc(content->num_pages,
  ...
  for(size=0, npages=0; ;npages++)
	...
	goto exit;
      ...
      fp->data = calloc(fp->length, 1);
      TRYF(ti73_recv_XDP((uint16_t *)&fp->length,fp->data)); fixup(fp->length);
      TRYF(ti73_send_ACK());
...
int ti73_get_idlist(char *id)
{
  ...
  TRYF(ti73_recv_XDP((uint16_t *)&varsize, id)); id[varsize] = '\0';
  TRYF(ti73_send_ACK());

...
static uint8_t pc_ti9x(void)
{
  switch(ticalcs_calc_type)
    {
    case CALC_TI89:  return PC_TI89;  break;
    case CALC_TI92P: return PC_TI92p; break;		
    case CALC_V200:  return PC_V200;  break;
    default:         return PC_TIXX;  break;
    }
  return 0;
}
...
int ti89_recv_CTS(void)
{
  ...
  if(length != 0x0000) return ERR_CTS_ERROR;
  DISPLAY(".\n");
...
int ti89_recv_XDP(uint32_t *length, uint8_t *data)
{
  ...
  DISPLAY(" TI->PC: XDP");
  err = recv_packet(&host, &cmd, (uint16_t *)length, data); fixup(*length);
...
{   
  ...
  if(cmd == CMD_EOT) return ERR_EOT;	// not really an error
  if(cmd != CMD_CONT) return ERR_INVALID_CMD;
  DISPLAY(".\n");
...
int ti89_recv_EOT(void)
{
  ...
  if(cmd != CMD_EOT) return ERR_INVALID_CMD;
  DISPLAY(".\n");
...
int ti89_recv_RTS(uint32_t *varsize, uint8_t *vartype, char *varname)
{
  ...
  *vartype = buffer[4];
  ...
  if(length != (6+strlen(varname))) return ERR_INVALID_PACKET; 

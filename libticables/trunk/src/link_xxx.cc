...
/**
 * ticables_cable_set_extra_options:
 * @handle: a previously allocated handle
 * @options_ex: pointer to structure containing cable extra options to be set
 *
...
TIEXPORT1 int TICALL ticables_cable_set_extra_options(CableHandle *handle, CableExtraOptions *options_ex)
{
	...
	handle->busy = 1;
	if (cable->set_extra_options)
	{
		CableEventData event;
		ticables_event_fill_header(handle, &event, /* type */ CABLE_EVENT_TYPE_BEFORE_GENERIC_OPERATION, /* retval */ 0, /* operation */ CABLE_FNCT_SET_EXTRA_OPTIONS);
		event.data.cptrval = (const void *)options_ex;
		ret = ticables_event_send(handle, &event);
		if (!ret)
		{
			ret = (cable->set_extra_options)(handle, options_ex);
		}
		ticables_event_fill_header(handle, &event, /* type */ CABLE_EVENT_TYPE_AFTER_GENERIC_OPERATION, /* retval */ ret, /* operation */ CABLE_FNCT_SET_EXTRA_OPTIONS);
		event.data.cptrval = (const void *)options_ex;
		ret = ticables_event_send(handle, &event);
	}
	handle->busy = 0;
...
/**
 * ticables_cable_get_extra_options:
 * @handle: a previously allocated handle
 * @options_ex: pointer to structure where cable options will be stored
 *
...
TIEXPORT1 int TICALL ticables_cable_get_extra_options(CableHandle *handle, CableExtraOptions *options_ex)
{
	...
	handle->busy = 1;
	if (cable->get_extra_options)
	{
		CableEventData event;
		ticables_event_fill_header(handle, &event, /* type */ CABLE_EVENT_TYPE_BEFORE_GENERIC_OPERATION, /* retval */ 0, /* operation */ CABLE_FNCT_GET_EXTRA_OPTIONS);
		event.data.cptrval = (const void *)options_ex;
		ret = ticables_event_send(handle, &event);
		if (!ret)
		{
			ret = (cable->get_extra_options)(handle, options_ex);
		}
		ticables_event_fill_header(handle, &event, /* type */ CABLE_EVENT_TYPE_AFTER_GENERIC_OPERATION, /* retval */ ret, /* operation */ CABLE_FNCT_GET_EXTRA_OPTIONS);
		event.data.cptrval = (const void *)options_ex;
		ret = ticables_event_send(handle, &event);
	}
	handle->busy = 0;

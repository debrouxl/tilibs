...
/* 
   ...
   things for him/her. Nethertheless, this fact has some consequences:
   - it is better (for USB & OS performances) to read/write a set of bytes 
   rather than uint8_t per uint8_t.
   - for reading, we have to read up to 32 bytes at a time (even if we need 
   only 1 uint8_t) and to store them in a buffer for subsequent acesses. 
   In fact, if we try and get uint8_t per uint8_t, it will not work.
   - for writing, we don't store bytes in a buffer. It seems better to send
   data uint8_t per uint8_t (latency ?!).
...
int ugl_open(void)
{
...
#ifdef HAVE_TILP_TICABLE_H
 {
   ...
   if(ioctl(dev_fd, IOCTL_TIUSB_TIMEOUT, arg) == -1)
     {
       DISPLAY_ERROR("Unable to use IOCTL codes.\n");
       return ERR_IOCTL;
     }
 }
#endif
...
int ugl_get(uint8_t *data)
{
  ...
  *data = *rBufPtr++;
  ...
  if(nBytesRead == -1) return ERR_READ_ERROR;
  if(nBytesRead == 0) return ERR_READ_TIMEOUT;
#endif
      ...
      for (dev = bus->devices; dev; dev = dev->next)
	{
	  if ((dev->descriptor.idVendor == TIGL_VENDOR_ID) && 
	      (dev->descriptor.idProduct == TIGL_PRODUCT_ID))
	    {
	      ...
	      DISPLAY("TIGL-USB found with libusb.\n");
...
static int enumerate_tigl_device(void)
{
  ...
  if (ret < 0)
    {
      DISPLAY_ERR("usb_find_devices: %s\n", usb_strerror());
      return ERR_LIBUSB_OPEN;
    }
...
int ugl_get2(uint8_t *data)
{
  ...
  if (nBytesRead <= 0)
      ...
      do
	{
	  ...
	  if(toELAPSED(clk, time_out)) return ERR_READ_TIMEOUT;
	  if(ret == 0)
	    DISPLAY_ERR("usb_bulk_read returns without any data. Retrying for circumventing quirk...\n");
	}
      while(!ret);
  ...
  *status = STATUS_NONE;
  ...
  if(tigl_han != NULL)
      ...
      do
	{
	  ...
	  if(toELAPSED(clk, time_out)) return ERR_READ_TIMEOUT;
	  if(ret == 0)
	    DISPLAY_ERR("usb_bulk_read returns without any data. Retrying...\n");
	}
      while(!ret);
...
TIGLUSB_OPEN		dynTiglUsbOpen	= NULL;	// Functions pointers for dynamic loading
...
int ugl_init()
{
	...
	if (hDLL == NULL)
	{
		DISPLAY_ERROR("TiglUsb library not found. Have you installed the TiglUsb driver ?\n");
		return ERR_OPEN_USB_DEV;
	}

	dynTiglUsbOpen = (TIGLUSB_OPEN)GetProcAddress(hDLL,
								    "TiglUsbOpen");
	if (!dynTiglUsbOpen)
	{
		DISPLAY_ERROR("Unable to load TiglUsbOpen symbol.\n");
	    FreeLibrary(hDLL);       
	    return ERR_FREELIBRARY;
	}

	dynTiglUsbFlush = (TIGLUSB_FLUSH)GetProcAddress(hDLL,
								    "TiglUsbFlush");
	if (!dynTiglUsbOpen)
	{
		DISPLAY_ERROR("Unable to load TiglUsbFlush symbol.\n");
	    FreeLibrary(hDLL);       
	    return ERR_FREELIBRARY;
	}

	dynTiglUsbRead= (TIGLUSB_READ)GetProcAddress(hDLL,
								    "TiglUsbRead");
	if (!dynTiglUsbRead)
	{
		DISPLAY_ERROR("Unable to load TiglUsbRead symbol.\n");
	    FreeLibrary(hDLL);       
	    return ERR_FREELIBRARY;
	}

	dynTiglUsbWrite= (TIGLUSB_WRITE)GetProcAddress(hDLL,
								    "TiglUsbWrite");
	if (!dynTiglUsbWrite)
	{
		DISPLAY_ERROR("Unable to load TiglUsbWrite symbol.\n");
	    FreeLibrary(hDLL);       
	    return ERR_FREELIBRARY;
	}

	dynTiglUsbClose = (TIGLUSB_CLOSE)GetProcAddress(hDLL,
								    "TiglUsbClose");
	if (!dynTiglUsbClose)
	{
		DISPLAY_ERROR("Unable to load TiglUsbClose symbol.\n");
	    FreeLibrary(hDLL);       
	    return ERR_FREELIBRARY;
	}

...
TIEXPORT1 const char *TICALL ticables_model_to_string(CableModel model)
{
	...
	case CABLE_SLV: return "UsbLink"; // for compat
...
TIEXPORT1 CableModel TICALL ticables_string_to_model(const char *str)
{
	if(!strcmp(str, "null"))
		...
		return CABLE_PAR;
	else if(!strcmp(str, "SilverLink")) // for compat
		return CABLE_USB;
	else if(!strcmp(str, "DirectLink")) // for compat
		return CABLE_USB;
		...
		return CABLE_TIE;
	else if(!strcmp(str, "TilEm"))
		return CABLE_TIE;
	else if(!strcmp(str, "TiEmu/TilEm"))
		return CABLE_TIE;

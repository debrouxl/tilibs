...
void DetectPorts9x(TicablePortInfo * pi)
{
  ...
  KeyCount = 0;
  while (RegEnumKeyEx(CurKey, KeyCount++, KeyName, &DummyLength,
		      NULL, NULL, NULL,
		      &DummyFileTime) != ERROR_NO_MORE_ITEMS) {
  ...
  for (KeyIndex = 0; KeyIndex < KeyCount; KeyIndex++) {
    ...
    if (RegOpenKeyEx
	(HKEY_DYN_DATA, KeyName, 0, KEY_PERMISSIONS,
	 &CurKey) != ERROR_SUCCESS)
      continue;
    ...
    if (DataType == REG_BINARY) {
      ...
      if (RegQueryValueEx(CurKey, PROBLEM, NULL,
			  NULL, Data, &DataSize) == ERROR_SUCCESS) {
	...
	HasProblem = TRUE;	// No good
      ...
      DataSize = MAX_PATH;
      RegQueryValueEx(CurKey, HARDWARE_KEY, NULL,
		      &DataType, HardwareSubKey, &DataSize);
      if (DataType != REG_SZ)
	HasProblem = TRUE;	// No good
	...
	if (RegOpenKeyEx
	    (HKEY_LOCAL_MACHINE, KeyName, 0,
	     KEY_PERMISSIONS, &CurKey) != ERROR_SUCCESS)
	  continue;

	// Now read in the PortName and obtain the LPT number from it
	DataSize = MAX_PATH;
	RegQueryValueEx(CurKey, PORTNAME, NULL,
			&DataType, PortName, &DataSize);
	if (DataType != REG_SZ)
	...
	if (strstr(PortName, "LPT") != NULL) {
	  ...
	  strncpy(PortNumberStr,
		  strstr(PortName,
			 "LPT") + 3,
		  strlen(PortName) -
		  (strstr(PortName, "LPT") - PortName) - 2);
	  ...
	  RegOpenKeyEx(HKEY_DYN_DATA,
		       KeyName, 0, KEY_PERMISSIONS, &CurKey);

	  DataSize = sizeof(Allocation);
	  RegQueryValueEx(CurKey, ALLOCATION,
			  NULL, &DataType, (unsigned char *)
			  Allocation, &DataSize);
	  if (DataType == REG_BINARY) {
	    ...
	    for (pos = 0; pos < 63; pos++)
	      if (Allocation[pos]
		  == 0x000C && Allocation[pos + 1]
		  != 0x0000 && PortNumber <= MAX_LPT_PORTS) {
		// Found a port; add it to the list
		FLPTAddress[PortNumber]
		    = Allocation[pos + 1];
		FLPTCount++;
	...
	if (strstr(PortName, "COM") != NULL) {
	  ...
	  strncpy(PortNumberStr,
		  strstr(PortName,
			 "COM") + 3,
		  strlen(PortName) -
		  (strstr(PortName, "COM") - PortName) - 2);
	  ...
	  if (DataType == REG_BINARY) {
	    ...
	    for (pos = 0; pos < 63; pos++)
	      if (Allocation[pos]
		  == 0x000C && Allocation[pos + 1]
		  != 0x0000 && PortNumber <= MAX_COM_PORTS) {
		// Found a port; add it to the list
		FCOMAddress[PortNumber]
		    = Allocation[pos + 1];
		FCOMCount++;
...
void DetectPortsNT1(TicablePortInfo * pi)
{
  ...
  const char *LOADED_KEY =
      "HARDWARE\\RESOURCEMAP\\LOADED PARALLEL DRIVER RESOURCES\\Parport";
  ...
  // Open the registry
  if (RegOpenKeyEx
      (HKEY_LOCAL_MACHINE, BASE_KEY, 0, KEY_PERMISSIONS,
       &CurKey) != ERROR_SUCCESS)
  ...
  ValueCount = 0;
  while (RegEnumValue(CurKey, ValueCount++, KeyName, &DummyLength,
		      NULL, &ValueType, NULL,
  ...
  for (index = 0; index < ValueCount; index++) {
    ...
    strcpy(KeyName, BASE_KEY);
    if (RegOpenKeyEx
	(HKEY_LOCAL_MACHINE, KeyName, 0, KEY_PERMISSIONS,
	 &CurKey) == ERROR_SUCCESS) {
      DataSize = MAX_PATH;
      RegQueryValueEx(CurKey, ValueList[index], NULL,
    ...
    if (strstr(DosDev, DOS_DEVICES) != NULL) {
      ...
      strncpy(PortNumberStr,
	      strstr(DosDev,
		     DOS_DEVICES) + strlen(DOS_DEVICES),
      ...
      strncpy(PortIDStr,
	      strstr(ValueList[index],
		     DEVICE_PARALLEL) +
	      strlen(DEVICE_PARALLEL), strlen(ValueList[index])
      ...
      RegOpenKeyEx(HKEY_LOCAL_MACHINE, LOADED_KEY, 0,
      ...
      if (RegQueryValueEx
	  (CurKey, DosDev, NULL, &DataType, NULL,
	   NULL) == ERROR_SUCCESS && DataType == REG_RESOURCE_LIST) {
	WORD Allocation[64];	// Binary data with port number inside

	// Read in the binary data
	DataSize = sizeof(Allocation);
	RegQueryValueEx(CurKey, DosDev, NULL, NULL, (unsigned char *)
			Allocation, &DataSize);
...
void DetectPortsNT2(TicablePortInfo * pi)
{
  ...
  const char *LOADED_KEY =
      "HARDWARE\\RESOURCEMAP\\LOADED SERIAL DRIVER RESOURCES\\Serial";
...
static int ticable_detect_mouse(TicablePortInfo * pi)
{
  ...
  if (found)
    ...
    DISPLAY(_
	    ("No PS/2 mouse found, the first serial port may be used by mouse.\r\n"));
    pi->com_addr[0] = 0;
  }
...
/* Windows part */
#ifdef __WIN32__
  {
    ...
    if (!strcmp(os, "Windows9x"))
      resources |= IO_ASM;
    DISPLAY(_("  IO_ASM: %s\r\n"),
	    resources & IO_ASM ? "ok" : "nok (PortTalk is needed)");

    // DRV is needed for NT platforms
  ...
  resources = IO_BSD | IO_API;
  DISPLAY(_("  IO_API: ok\r\n"));

  {				// check super or normal user
    uid_t uid = getuid();
    if (uid != 0) {
      DISPLAY(_("  IO_ASM: nok\r\n"));
      resources &= ~IO_ASM;
    } else {
      DISPLAY(_("  IO_ASM: ok (super user)\r\n"));
      resources |= IO_ASM;
